...
smallStencil = bwdist(smallStencil);%计算smallStencil中每个元素距最近非0元素的距离

%% labelPixels
[labeled, nred, nblue] = labelRBPix( filename, withFilter );%图像处理+连通域标记
%[所有连通块,R数目,B数目]
tmpLabeled = labeled;
...
numlabels = nred + nblue;%总连通块数目
floodGraph = cell(numlabels, 5);
for label = 1:numlabels
    % Graph of Neighbors
    currLabelPix = labeled == label;%获取当前想要找的连通块label的mask
    neighbors = imdilate(currLabelPix, nHood) & ~currLabelPix;%dilated mask & ~ mask
    
    neighborLabels = unique( labeled(neighbors) );%得到相邻连通块的label
    neighborLabels( neighborLabels == 0 ) = []; % remove boundary
	%floodGraph{label, 1}记录邻居
    ...
    for n = 1:length(neighborLabels)%遍历相邻label
        neighbor = neighborLabels(n);
        
        [rows, cols] = find( labeledNeighbors == neighbor );%查找非0元素
        ...
        for s = 1:numSamples
            ...
            pixelFound = findWithStencil( labeled, center, label, smallStencil);%找最近邻居
            if pixelFound(1) ~= -1
...
adjMat = zeros(numlabels, numlabels);%邻接矩阵
...
for sEL = 1:length(startLabels)
    ...
    labelToVisit.add(startLabel);%入队
	%队列非空
    ...
    for potPath = 1:length(allPaths)
        if length( allPaths{potPath,1} ) == codeLength + 4
            ...
            for p = 1:length(pathCode)
                ...
                val = distPairMap(keyPair);%keyPair与logRatio的映射先前已建好
                potentialPaths{validPathIdx,2}(p,1) = val;
                if val < -0.1249 || val > 0.1761 % threshold clusters
                    potentialPaths{validPathIdx,3}(p,1) = 1;%码元1
                else
                    potentialPaths{validPathIdx,3}(p,1) = 0;%码元0
                end
            end
            
            % flip direction(有可能整个码串读反了，因为logRatio不区分1/M和M两种比例)
            if potentialPaths{validPathIdx,3}(2,1) == 1 && potentialPaths{validPathIdx,3}(end,1) == 0
                potentialPaths{validPathIdx,1} = flipud( potentialPaths{validPathIdx,1} );%反转数组
